...
On **line 1**, we import the [`describe`](https://serenity-js.org/api/playwright-test/function/describe/)
...
On **lines 3 to 5**, we import Serenity/JS Screenplay Pattern libraries that we'll use to interact with the web app
...
**Lines 9 to 29** define our first test scenario, interacting with the [nested frames](https://the-internet.herokuapp.com/nested_frames)
...
On **line 21**, to perform activities in the context of a frame, we use the interaction to [`Switch`](https://serenity-js.org/api/web/class/Switch/) from the
...
A common scenario where you'll likely run into frames involves working with <abbr title="What You See Is What You Get">WYSIWYG</abbr> editors, such as [TinyMCE](http://www.tinymce.com/).

**Lines 31 to 62** define our second test scenario, interacting with the [TinyMCE editor embedded in a nested frame](https://the-internet.herokuapp.com/iframe).
...
Once the page loads:
- on **line 48**: we switch into the frame that contains TinyMCE
- on **line 49**: store the original text displayed in the editor and store it in the actor's notes
- on **line 51**: use the interaction to [`Clear`](https://serenity-js.org/api/web/class/Clear/) to remove the contents of the editor
- on **line 52**: use the interaction to [`Enter`](https://serenity-js.org/api/web/class/Enter/) to enter the new value
- on **line 55**: use the interaction to [`Ensure`](https://serenity-js.org/api/assertions/class/Ensure/) to assert that the new value is different than what the actor saw originally

When the activity sequence from **lines 48 to 58** is complete, the actor automatically switches their focus back to the top-level page.
This mechanism allows us to go straight to the next step of the scenario without having to worry about the frames, and so:
- on **line 59**: ensure that the content of the top-level page heading is as expected
